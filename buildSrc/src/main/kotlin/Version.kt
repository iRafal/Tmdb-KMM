...
class Version(
    ...
    val name: String
        get() {
            return when (suffix) {
                is Suffix.RC -> {
                    require(suffix.number > 0, lazyMessage = { "Wrong suffix name: $suffix" })
                    "$versionBasePart-${suffix.name}"
                }

                is Suffix.Custom -> {
                    require(suffix.name.isNotBlank(), lazyMessage = { "Wrong suffix name: $suffix" })
                    "$versionBasePart-${suffix.name}"
                }
    ...
    private val versionBasePart: String
        get() {
            return if (patch == null) {
                ...
                require(patch >= 0, lazyMessage = { "Wrong patch value: $patch" })
                "$major.$minor.$patch"
            }
        }
}
